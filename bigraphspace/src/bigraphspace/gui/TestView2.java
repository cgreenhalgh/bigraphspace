...
import reactivexml.model.DomBigraph;
import reactivexml.model.SignatureFactory;

import org.w3c.dom.Document;
import org.w3c.dom.Element;

import bigraphspace.model.*;
 ...
 *
 */
public class TestView2 implements java.awt.image.ImageObserver, MouseListener {
	/** logger */
	static Logger logger = Logger.getLogger(TestView2.class);
	...
	public static void main(String[] args) {
		new TestView2().doMain(args);	
	}
	/** cons */
	TestView2() {
		try {
			bigraph = DomBigraph.newBigraph(basicSignature);
		}
		catch (Exception e) {
			logger.error("Creating bigraph", e);
		}
	}
	/** frame */
...
//					paintBigraph((Graphics2D)graphics);
	...
	protected void insertRoot() {
		Place root = bigraph.createRoot();
		bigraph.addRoot(root);
...
//		if (parent==null)
//			return;
		Set<Control> controls = basicSignature.getControls();
		...
		controlList.setCellRenderer(new DefaultListCellRenderer() {
			...
			public Component getListCellRendererComponent(JList list,
				...
				if (value instanceof Control)  {
					...
					return super.getListCellRendererComponent(list, control.getName()+" ("+control.getStatus()+", arity "+control.getArity()+")", index, isSelected,
							cellHasFocus);
				}
		...
		Place node = bigraph.createNode(control);
//		Node node = new Node();
...
//	}
	...
	protected Element getSelectedPlace() {
		// selected place?!
		...
		if (selections.get(0) instanceof Element) {
			return (Element)selections.get(0);
		}
		JOptionPane.showMessageDialog(frame, "Select exactly one place", "Error", JOptionPane.ERROR_MESSAGE);
		return null;
	}
	...
	Bigraph bigraph;
	/** interpreter state */
//	protected HashMap<String,XmlModel> interpreterVariables = new HashMap<String,Bigraph>();
	/** view program */
	protected boolean viewProgram(String script, JComponent canvas) {
		try {
			...
			BigraphTermLexer lex = new BigraphTermLexer(new ANTLRStringStream(script));
			...
			if (tree instanceof CommonTree) {
				...
				bigraph = new DomBigraph(this.basicSignature, doc);
				bigraph.dump(System.out);
			}
			if (bigraph==null)
				return false;
...
//		}
	...
	/** make view, recurse */
//	protected PlaceView makeView(Element place) {
//		List<Element> children = bigraph.getChildren(place);
...
//	}
	...
	protected void loadSignature() {
	    ...
	    try {
	    	...
	    	basicSignature = SignatureFactory.readSignature(f);
	    	bigraph.setSignature(basicSignature);
